...
static void drawlogo(Monitor *m);
static void clearlogo(Monitor *m);
...
int drawstatusbar(Monitor *m, int bh, char *stext) {
  ...
  if (workspaces_locked) {
    if (asprintf(&display_text, "🔒 %s", stext) == -1)
      die("asprintf");
  } else {
    display_text = stext;
  }
...
void drawlogo(Monitor *m) {
  ...
  XDrawString(drw->dpy, root, drw->gc, m->wx + (m->ww - TEXTW("dwm")) / 2,
              m->wy + (m->wh + drw->fonts->h) / 2, "dwm", 3);
}

void clearlogo(Monitor *m) {
  XSetForeground(drw->dpy, drw->gc, scheme[SchemeNorm][ColBg].pixel);
  XFillRectangle(drw->dpy, root, drw->gc, m->wx + (m->ww - TEXTW("dwm")) / 2,
                 m->wy + (m->wh - bh) / 2, TEXTW("dwm"), bh);
}
...
void focus(Client *c) {
  ...
  if (c) {
    if (c->mon != selmon) {
      ...
      {
        Client *cc;
        for (cc = selmon->stack; cc; cc = cc->snext)
          XRaiseWindow(dpy, cc->win);
      }
    }
    if (c->isurgent)
      seturgent(c, 0);
...
int updategeom(void) {
  ...
  /* query xrandr for screen resources */
  ...
  for (i = 0, j = 0, m = mons; i < sr->ncrtc && m; i++) {
    ci = XRRGetCrtcInfo(dpy, sr, sr->crtcs[i]);
    if (!ci || ci->noutput == 0) {
      if (ci)
        XRRFreeCrtcInfo(ci);
    ...
    if (ci->x != m->mx || ci->y != m->my || ci->width != m->mw ||
        ci->height != m->mh) {
      dirty = 1;
      m->num = j;
      m->mx = m->wx = ci->x;
  ...
  /* remove excess monitors if n > nn */
  for (i = nn; i < n; i++) {
    for (m = mons; m && m->next; m = m->next)
      ;
    while ((c = m->clients)) {
